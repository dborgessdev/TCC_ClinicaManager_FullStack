...
from ..serializers import ReceptionSerializer, QueueSerializer
...
def iniciar_triagem_view(request):
    '''Atualiza o status da fila para 'triagem' e redireciona para a ficha de triagem.'''
    ...
    # Redireciona para a página de triagem com a senha da fila
    return redirect('ficha_triagem', senha=queue.senha)

def formulario_triagem_view(request, senha):
    '''Exibe o formulário de triagem para o paciente associado a uma senha.'''
    queue = get_object_or_404(Queue, senha=senha)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            # Atualiza o status da fila para 'pre_atendimento'
            queue.status = 'pre_atendimento'
            queue.save()

            # Cria o objeto Reception e associa à fila
            reception = form.save(commit=False)
            reception.queue = queue

            # Salva o objeto Reception
            reception.triagem_finalizada = True
            reception.save()

            # Redireciona para a página de pré-triagem
            return redirect('/api/pre-triagem/')  
    else:
        form = TriagemForm()

    # Renderiza o template com o formulário e os dados do paciente
